            ...
            <div className="space-y-3.5 max-h-[70vh] overflow-y-auto pr-1">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full h-8.5 bg-[#0d1017] border border-[#28324a] rounded-xl px-2.5 text-xs text-white focus:outline-hidden focus:border-pink-500 cursor-pointer"
                >
                  <option value="">-- অথবা ড্রপডাউন থেকে সিলেক্ট করুন --</option>
                  {SHEET3_PRIMARY_PRODUCTS.map((p) => (
                    <option key={p} value={p}>
                      {p}
                    </option>
                  ))}
                  {editProductName &&
                    !SHEET3_PRIMARY_PRODUCTS.some(
                      (p) => p.toLowerCase().trim() === editProductName.toLowerCase().trim()
                    ) && (
                      <option value={editProductName}>
                        {editProductName} (বর্তমান রো-এর নাম)
                      </option>
                    )}
                </select>
              </div>
      ...
      {isNewStockOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-xs animate-fadeIn">
          <div className="bg-[#121520] border border-[#252c42] rounded-2xl w-full max-w-md overflow-hidden shadow-2xl p-5 space-y-4">
            ...
            {/* Switch between Sheet 3 Direct Row Entry, Existing Product Restock, and New Product */}
            <div className="flex items-center gap-1.5 p-1 bg-[#0d1017] rounded-xl border border-[#242c3f] overflow-x-auto">
              <button
                type="button"
                onClick={() => setNewStockMode('sheet3_direct')}
                className={`flex-1 py-1.5 px-2 text-[11px] font-semibold rounded-lg whitespace-nowrap transition-all cursor-pointer ${
                  newStockMode === 'sheet3_direct'
              ...
              <button
                type="button"
                onClick={() => setNewStockMode('existing_product')}
                className={`flex-1 py-1.5 px-2 text-[11px] font-semibold rounded-lg whitespace-nowrap transition-all cursor-pointer ${
                  newStockMode === 'existing_product'
                    ? 'bg-pink-600 text-white shadow-sm'
                    : 'text-gray-400 hover:text-gray-200'
                }`}
              >
              ...
              <button
                type="button"
                onClick={() => setNewStockMode('new_product')}
                className={`flex-1 py-1.5 px-2 text-[11px] font-semibold rounded-lg whitespace-nowrap transition-all cursor-pointer ${
                  newStockMode === 'new_product'
                    ? 'bg-pink-600 text-white shadow-sm'
                    : 'text-gray-400 hover:text-gray-200'
                }`}
              >
            ...
            {/* Mode 1: Direct Sheet 3 Row Entry */}
            {newStockMode === 'sheet3_direct' && (
              <div className="space-y-3.5 max-h-[60vh] overflow-y-auto pr-1">
                {/* 6 Products Toggle Buttons */}
                <div>
                  <label className="text-xs font-semibold text-gray-300 block mb-1.5 flex items-center justify-between">
                  ...
                  {/* Dropdown fallback */}
                  <select
                    value={sheet3NewProductName}
                    onChange={(e) => handleSelectSheet3NewProduct(e.target.value)}
                    className="w-full h-8.5 bg-[#0d1017] border border-[#28324a] rounded-xl px-2.5 text-xs text-white focus:outline-hidden focus:border-pink-500 cursor-pointer"
                  >
                    {SHEET3_PRIMARY_PRODUCTS.map((p) => (
                      <option key={p} value={p}>
            ...
            {/* Mode 2: Existing Product Restock */}
            {newStockMode === 'existing_product' && (
              <div className="space-y-3">
                <div>
            ...
            <div className="flex items-center gap-2 pt-2 border-t border-[#1f2537]">
              ...
              <button
                type="button"
                onClick={() => {
                  if (newStockMode === 'sheet3_direct') {
                    handleSaveDirectSheet3Entry();
                  } else {
                    if (newStockMode === 'new_product') {
                      setIsCreatingNewProduct(true);
                    } else {
                      setIsCreatingNewProduct(false);
                    }
                    handleSaveNewStockEntry();
                  }
                }}
                disabled={isAddingSheet3Entry}
                className="flex-1 py-2.5 rounded-xl bg-gradient-to-r from-pink-600 to-rose-600 hover:from-pink-500 hover:to-rose-500 text-white text-xs font-bold shadow-md shadow-pink-600/30 flex items-center justify-center gap-1.5 cursor-pointer disabled:opacity-50"
              >
                {isAddingSheet3Entry ? (
                  ...
                  <>
                    <Check className="w-3.5 h-3.5" />
                    <span>{newStockMode === 'sheet3_direct' ? 'শিট ৩-এ রো যোগ করুন' : 'স্টক সংরক্ষণ করুন'}</span>
                  </>
                )}
              </button>
            </div>
